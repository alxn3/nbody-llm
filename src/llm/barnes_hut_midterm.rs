...
#[derive(Clone)]
pub struct BarnesHutSimulationMidterm<F: Float, const D: usize, P, I = LeapFrogIntegrator<F, D, P>>
where
    P: Particle<F, D>,
    I: Integrator<F, D, P>,
{
...
impl<F: Float, const D: usize, P, I> BarnesHutSimulationMidterm<F, D, P, I>
where
    ...
    F: Send + Sync,
{
        ...
        match node_idx {
            Some(node_idx) => {
                assert!(self
                    ...
                    .get(node_idx)
                    .bounds
                    .contains(self.points[point_index].position()));
                ...
                match self.arena.get(node_idx).node_data {
                    NodeData::PointIndex(index) => {
                        let o1 = self
                            ...
                            .get(node_idx)
                            .bounds
                            .get_orthant(self.points[index].position());
                        let o2 = self
                            .arena
                            .get(node_idx)
                            .bounds
                            .get_orthant(self.points[point_index].position());
    ...
    fn build_tree(&mut self) {
        ...
        self.previous_positions
            .extend(self.points.iter().map(|p| *p.position()));
    ...
    fn update_tree(&mut self) {
        ...
        if F::from(moved_indices.len()).unwrap()
            > rebuild_fraction * F::from(self.points.len()).unwrap()
        {
            self.build_tree();
            return;
        }
    ...
    fn calc_force(&self, node_idx: usize, point: &P) -> SVector<F, D> {
        ...
        let point_idx = match point {
            ...
            p if self.points.len() > 1
                && std::ptr::eq(p as *const P, &self.points[1] as *const P) =>
            {
                Some(1)
            }
            _ => None,
...
impl<F: Float, const D: usize, P, I> Simulation<F, D, P, I>
    for BarnesHutSimulationMidterm<F, D, P, I>
where
    P: Particle<F, D> + Send + Sync,
    I: Integrator<F, D, P> + Sync,
    F: Send + Sync,
...
#[cfg(feature = "render")]
impl<F, P, I> Renderable for BarnesHutSimulationMidterm<F, 3, P, I>
where
    F: Float + Send + Sync,
